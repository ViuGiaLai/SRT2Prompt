      ...
      <section className="mx-auto grid min-h-screen max-w-6xl items-center gap-10 px-5 py-10 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="rounded-lg border border-line bg-panel p-6">
          ...
          <div className="mt-5 text-sm">
            <Link href="/login" className="text-muted hover:text-white">Already have an account?</Link>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-line bg-panel p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {["Summary", "Scene prompts", "Thumbnail", "Titles", "Description", "Hashtags"].map((item) => (
              <div key={item} className="rounded-md border border-line bg-panelSoft p-3 text-sm text-gray-200">
                {item}
              </div>
            ))}
          </div>
